...
def invoiceToDict(invoice):
    ...
    simple_invoice['date_opened'] = invoice.GetDateOpened().strftime(
        '%Y-%m-%d')
    simple_invoice['date_posted'] = (
        None
        if invoice.GetDatePosted().strftime('%Y-%m-%d') == '1970-01-01'
        else invoice.GetDatePosted().strftime('%Y-%m-%d')
    )
...
def billToDict(bill):
    ...
    simple_bill['date_posted'] = (
        None
        if bill.GetDatePosted().strftime('%Y-%m-%d') == '1970-01-01'
        else bill.GetDatePosted().strftime('%Y-%m-%d')
    )
...
def entryToDict(entry):
    ...
    simple_entry['inv_account'] = (
        ...
        else accountToDict(entry.GetInvAccount())
    )
...
def accountToDict(account):
    ...
    simple_account['currency'] = (
        ...
        else account.GetCommodity().get_mnemonic()
    )

    simple_account['subaccounts'] = [
        accountToDict(subaccount)
        for subaccount in account.get_children_sorted()
    ]
